...
public class PairMatchingResult {
    ...
    private List<Set<Crew>> handleCrewsWithEvenNumber(List<Crew> shuffledCrews) {
    ...
    private List<Set<Crew>> handleCrewsWithOddNumber(List<Crew> shuffledCrews) {
        List<Set<Crew>> result = new ArrayList<>();
    ...
    private Set<Crew> getLastPair(List<Crew> shuffledCrews) {
    ...
    private List<Crew> toCrew(List<Crew> crews) {
        return Randoms.shuffle(toName(crews)).stream()
                .map(Crews::findCrewByName)
                .collect(Collectors.toList());
    }

    private List<String> toName(List<Crew> crews) {
        return crews.stream().map(Crew::getName).collect(Collectors.toList());
    }

    private boolean isCrewNumberEven(List<Crew> shuffledCrews) {
